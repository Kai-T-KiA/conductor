  ...
  module V1
    class WorkHoursController < BaseController
      ...
      def create
        ...
        Rails.logger.debug "Work Hour Valid? #{@work_hour.valid?}"
        if !@work_hour.valid?
          Rails.logger.debug "Validation Errors: #{@work_hour.errors.full_messages}"
        end
      ...
      # # createメソッド時に許可されたパラメータのみを取得するストロングパラメータ設定
      # def work_hour_params_of_create
      #   # work_hourパラメータの中から許可された属性のみを抽出
      #   params.require(:work_hour).permit(
      #     :work_date, :start_time, :end_time, :hours_worked, :activity_description, :user_id
      #   )
      # end
